...
# ============================================
# MDKenZnhoTT - Auto Rejoin Tool v3.0 (root)
# Menu trung tâm + nhận diện client thủ công
# ============================================
...
CONF="$SCRIPT_DIR/MDKenZnhoTT.conf"
...
get_status(){
  ...
  # Sử dụng presence.roproxy.com (proxy cho API chính thức)
  local API_URL="https://presence.roproxy.com/v1/presence/users"
  local body
  body=$(printf '{"userIds":[%s]}' "$uid")

  local resp
  resp=$(curl -s -X POST -H "Content-Type: application/json" -d "$body" "$API_URL" --max-time 8) || true
  if [ -z "$resp" ]; then
    echo "OFFLINE"
    return
  fi
  ...
  local pres
  pres=$(echo "$resp" | jq -r '.userPresences[0].userPresenceType // 0')
  case "$pres" in
    0) echo "OFFLINE" ;;
    1) echo "ONLINE" ;;
    2|3) echo "IN-GAME" ;;
...
monitor_loop(){
  ...
  while true; do
    ...
    for i in "${!CLIENTS[@]}"; do
      ...
      # Logic xử lý: nếu offline -> restart luôn
      if [ "$status" = "OFFLINE" ]; then
        restart_one_pkg "$pkg" "OFFLINE"
      elif [ "$status" = "ONLINE" ]; then
        # Có thể lỗi 267 -> restart an toàn
...
setup_clients(){
  ...
  while true; do
    read -p "Nhập client của bạn (hoặc n để kết thúc): " client
    if [ "$client" = "n" ]; then
      break
    fi
...
menu(){
  logo
  echo -e "${CYAN}Version 3.0 | Created by KenZnhoTT${NC}"
  echo -e "${YELLOW}+----+--------------------------------------+${NC}"
...
while true; do
  ...
  case $choice in
    1)
      if [ -z "${CLIENTS[*]}" ]; then
        echo "Chưa có client nào! Vui lòng thiết lập trước."
        sleep 2
        continue
      fi
      monitor_loop
